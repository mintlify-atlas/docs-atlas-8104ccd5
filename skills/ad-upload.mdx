---
title: "Ad Upload"
description: "Push ad copy and images to Meta via Graph API — no Ads Manager required. Uploads images, builds asset_feed_spec creatives, and creates or refreshes ads."
icon: "cloud-arrow-up"
---
...
<ParamField path="workspace/brand/assets.md" type="optional">
...
<ParamField path="workspace/brand/assets.md">
  Appends uploaded creative IDs and ad IDs
</ParamField>

<ParamField path="workspace/campaigns/{name}/ads/{creative}.upload.json">
...
| Body | 50-500 chars per variant | "Body V1 too short (under 50 chars)" |
...
<Note>
3 images × 3 headlines × 3 bodies = 27 combinations. Meta's algorithm finds the winners.
</Note>
...
<Note>
You're replacing the creative on the existing ad. The ad ID stays the same (metrics history preserved). The old creative still exists — it's just detached from this ad.
</Note>
...
- **Upstream (required):** `ad-copy-generator` — produces the asset_feed_spec JSON and copy variants
- **Upstream (trigger):** `ad-creative-monitor` — detects creative fatigue and flags ads that need copy refresh
- **Downstream:** `meta-ads` — monitor performance of the ads this skill just created
...
<ParamField path="FACEBOOK_ACCESS_TOKEN" type="required">
  Facebook access token with `ads_management` scope
</ParamField>

<ParamField path="META_AD_ACCOUNT" type="required">
  Meta ad account ID (format: `act_123456789`)
</ParamField>

```bash
# Set from social-cli config
export FACEBOOK_ACCESS_TOKEN=$(jq -r '.profiles.default.tokens.facebook' ~/.social-cli/config.json)
export META_AD_ACCOUNT="act_123456789"
```
...
- **[Ad Copy Generator](/skills/ad-copy-generator)** — Generate the asset_feed_spec JSON that feeds this skill
- **[Meta Ads](/skills/meta-ads)** — Monitor performance of uploaded ads
- **[Ad Creative Monitor](/skills/ad-creative-monitor)** — Detect when ads need copy refresh
